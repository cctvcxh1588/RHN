...
export async function GET(request: Request) {
  try {
    ...
    let query = supabase.from("cms_schedule").select("*").order("sort_order", { ascending: true });
    if (!isAdmin) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const supabase = getSupabaseClient();
    const { data, error } = await supabase.from("cms_schedule").insert(body).select().single();
    if (error) throw error;
